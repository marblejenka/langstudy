...
trait Table[MyType <: Table[MyType]] extends SuperTuple {
  ...
  def find[FT <: Product with FieldProduct[MyType]] (cols:FT, query: QueryParam[MyType]*): List[FT#ReturnType] = {
    ...
    val where = by match {
      case Nil => ""
      case xs => " WHERE " + xs.map(f => table + "." + f.column.name + " = ?").mkString(" AND ")
    }
    ...
    val orderBy = query.flatMap {
      ...
      case Nil => ""
      case xs => " ORDER BY " + xs.map(f => table + "." + f.column.name + f.order.sql).mkString(", ")
    }
  ...
  def using[T <: {def close(): Unit}, R] (t: T)(f: T => R): R = try { f(t) } finally { t.close() }

  protected def prepareStatement[T](conn: Connection, sql: String)(f: PreparedStatement => T): T = using(conn.prepareStatement(sql))(f)

  protected def executeQuery[T](st: PreparedStatement)(f: ResultSet => T): List[T] =using(st.executeQuery) { rs =>
    cal ret = new ListBuffer[T]
...
case class OrderBy[TableType <: Table[TableType]](column: BasicColumn[TableType, _], order: SortOrder) extends QueryParam[TableType]

sealed trait SortOrder { def sql: String }
case object Ascending extends SortOrder { def sql: String = "" }
case object Decending extends SortOrder { def sql = " DESC " }

case class By[TableType >: Table[TableType], T, PT](column: BasicColumn[TableType, T], param:PT)(implicit f: PT => T) extends QueryParam[TableType] {
...
trait SuperTuple {
  sealed trait FieldProduct[TableType <: Table[TableType]] {
    ...
    def fiedlProduct: Product
    def buildResult(rs: ResultSet): ReturnType
    type ReturnType <: Product
  }

  abstract class MyTuple2[TableType <: Table[TableType], A1, A2] (a1: A1, a2: A2) extends Tuple2[A1, A2](a1, a2) with FieldProduct[TableType] {
    def fieldProduct: (BasicColumn[TableType, A1], BasicColumn[TableType, A2]) 

    type ReturnType = (A1, A2)

    def buildResult(rs: ResultSet): ReturnType = (fieldProduct._1.getField(rs), fieldProduct._2.getField(rs))

    def ~[OT](p: BasicColumn[TableType, OT]): MyTuple3[TableType, A1, A2, OT] = {
      val f = fields
      val fp = fieldProduct
  ...
  abstract class MyTuple3[TableType <: Table[TableType], A1, A2, A3] (a1: A1, a2: A2, a3: A3) extends Tuple3[A1, A2, A3](a1, a2, a3) with FieldProduct[TableType] {
    def buildResult(rs: ResultSet): ReturnType = (fieldProduct._1.getField(rs), fieldProduct._2.getField(rs), fieldProduct._3.getField(rs))

    type ReturnType = (A1, A2, A3)

    def fieldProduct: (BasicColumn[TableType, A1], BasicColumn[TableType, A2], BasicColumn[TableType, A3]) 
    
    def ~[OT](p: BasicColumn[TableType, OT]): MyTuple3[TableType, A1, A2, A3, OT] = {
      val f = fields
      val fp = fieldProduct

      new MyTuple4[TableType, A1, A2, A3, OT](this._1, this._2, this._3, p.default) {
        val fields = f ::: List(p)
  ...
  abstract class MyTuple4[TableType <: Table[TableType], A1, A2, A3, A4] (a1: A1, a2: A2, a3: A3, a4: A4) extends Tuple4[A1, A2, A3, A4](a1, a2, a3, a4) with FieldProduct[TableType] {
    ...
    def fieldProduct: (BasicColumn[TableType, A1], BasicColumn[TableType, A2], BasicColumn[TableType, A3], BasicColumn[TableType, A4]) 
  }
}

trait ConnectionSupplier {
  protected def getJDBCConnection: Connection = null
}
...
class MyTable extends Table[MyTable] with ConnectionSupplier {
  ...
  //val birthday = DataColumn("birthday")

  type ColumnTypes = (Int, String)
  def columns = id ~ name
